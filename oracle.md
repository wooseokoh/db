...
- 클러스터드 인덱스(Clustered Index)와 넌클러스터드 인덱스(Nonclustered Index)

```
클러스터드 인덱스(Clustered Index):

- 테이블의 데이터를 물리적으로 정렬하는 인덱스입니다.
- 한 테이블당 하나의 클러스터드 인덱스만 생성할 수 있습니다.
- 클러스터드 인덱스는 테이블의 프라이머리 키(PK)로 설정된 열에 대해 자동으로 생성됩니다.
- 데이터는 클러스터드 인덱스의 순서에 따라 저장되며, 클러스터드 인덱스의 순서와 테이블의 물리적 순서가 일치합니다.
- 클러스터드 인덱스를 기반으로 데이터를 검색하거나 정렬하는 작업은 효율적입니다.
- 인덱스 자체가 테이블의 구조이기 때문에 인덱스의 수정은 데이터의 재정렬을 필요로 합니다.

넌클러스터드 인덱스(Nonclustered Index):

- 테이블의 데이터를 별도의 구조로 정렬하는 인덱스입니다.
- 한 테이블당 여러 개의 넌클러스터드 인덱스를 생성할 수 있습니다.
- 넌클러스터드 인덱스는 테이블의 프라이머리 키나 일반 열에 대해 생성할 수 있습니다.
- 데이터는 넌클러스터드 인덱스의 순서에 따라 정렬되지 않으며, 인덱스는 키 값과 해당 키 값을 가진 레코드의 위치를 매핑하는 역할을 합니다.
- 넌클러스터드 인덱스를 기반으로 데이터를 검색하거나 정렬하는 작업은 추가적인 인덱스 탐색이 필요하므로 클러스터드 인덱스보다는 상대적으로 느릴 수 있습니다.
- 넌클러스터드 인덱스는 테이블의 구조에 영향을 주지 않고 인덱스를 수정할 수 있습니다.

클러스터드 인덱스 예시:
가정하자면, "학생"이라는 테이블이 있고, 이 테이블은 "학번"이라는 열을 기반으로 클러스터드 인덱스를 가지고 있다고 가정해봅시다. 클러스터드 인덱스는 데이터를 물리적으로 정렬하기 때문에 "학번"을 기준으로 학생 데이터가 정렬됩니다. 이 경우, 테이블의 물리적 순서와 클러스터드 인덱스의 순서가 일치하게 됩니다.

넌클러스터드 인덱스 예시:
위의 "학생" 테이블을 기준으로 넌클러스터드 인덱스 예시를 살펴보겠습니다. 이번에는 "성적"이라는 열을 기준으로 넌클러스터드 인덱스를 생성한다고 가정해봅시다. 넌클러스터드 인덱스는 테이블의 데이터를 별도의 구조로 정렬합니다. 따라서 "성적"을 기준으로 넌클러스터드 인덱스를 생성하면 해당 인덱스는 "성적" 값과 해당 값에 대응하는 학생 레코드의 위치를 매핑합니다. 이 때, 테이블의 물리적 순서와 넌클러스터드 인덱스의 순서는 일치하지 않을 수 있습니다.

즉, 클러스터드 인덱스는 테이블의 레코드를 물리적으로 정렬하고, 넌클러스터드 인덱스는 별도의 인덱스 구조로 데이터를 정렬합니다. 클러스터드 인덱스는 한 테이블당 하나만 생성할 수 있지만, 넌클러스터드 인덱스는 여러 개 생성할 수 있습니다.
```

- 프라이머리 인덱스(Primary Index)와 세컨더리 인덱스(Secondary Index)

```
프라이머리 인덱스 예시:
가정하자면, "도서"라는 테이블이 있고, 이 테이블은 "도서번호"라는 열을 프라이머리 키로 가지고 있다고 가정해봅시다. 프라이머리 인덱스는 테이블의 레코드를 고유하게 식별하기 위한 인덱스입니다. 이 경우, "도서번호" 열을 기반으로 프라이머리 인덱스가 생성되고, 도서 테이블의 레코드는 프라이머리 인덱스에 따라 고유하게 식별됩니다.

세컨더리 인덱스 예시:
위의 "도서" 테이블을 기준으로 세컨더리 인덱스 예시를 살펴보겠습니다. 이번에는 "도서저자"라는 열을 세컨더리 인덱스로 생성한다고 가정해봅시다. 세컨더리 인덱스는 프라이머리 인덱스 이외의 열을 기반으로 생성됩니다. 따라서 "도서저자"를 기준으로 세컨더리 인덱스를 생성하면 해당 인덱스는 "도서저자" 값과 해당 값에 대응하는 도서 레코드의 위치를 매핑합니다. 세컨더리 인덱스는 프라이머리 인덱스와 별개로 존재하며, 도서 테이블의 레코드를 "도서저자" 값을 기준으로 검색 및 정렬할 수 있게 합니다.

즉, 프라이머리 인덱스는 테이블의 레코드를 고유하게 식별하기 위한 인덱스이며, 세컨더리 인덱스는 프라이머리 인덱스 이외의 열을 기반으로 생성되는 인덱스입니다. 프라이머리 인덱스는 테이블당 하나만 생성할 수 있고, 세컨더리 인덱스는 여러 개 생성할 수 있습니다.
```
